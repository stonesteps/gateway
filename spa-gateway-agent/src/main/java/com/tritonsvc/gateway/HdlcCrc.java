...
/**
 * helper utility to do hdlc crc
 */
public class HdlcCrc {
    private static final int HDLC_FCS8_VALID = 0xF3;
    ...
    private static int calculateHdlcCrc8(byte[] input, int start, int stop) {
        int hdlccrc8 = 0xFF;

        for (int x =start; x >= start && x <stop; x++) {
            int tempCrc = (0xFF & input[x]) ^ hdlccrc8;
            hdlccrc8 = crc8table[tempCrc];
        }
        return hdlccrc8;
    }

    /**
     * validate the fcs that is inline in the byte array,
     * the byte array should only be the app message, no leading/trailing flag delimeters should be here
     *
     * @param input
     * @return
     */
    public static boolean isValidFCS(byte[] input) {
        // read the entire message block between flags
        return calculateHdlcCrc8(input, 0, input.length) == HDLC_FCS8_VALID;
    }

    /**
     * generate the fcs from a full message byte array, that has beginning flag delimiter
     * and has not had fcs byte or trailing flag delimiter put in yet
     *
     * @param input
     * @return
     */
    public static byte generateFCS(byte[] input) {
        int hdlcLength = 0xFF & input[1];
        // only use message block up to the fcs byte position
        return (byte)(calculateHdlcCrc8(input, 1, hdlcLength) ^ 0xFF);
    }
}
